...
export default function Aurora(props: AuroraProps) {
  ...
  useEffect(() => {
    ...
    let stopsUniform = colorStopsArray;
    const getStopsUniform = () => {
      const stops = propsRef.current.colorStops ?? colorStops;
      if (stops !== lastStops) {
        lastStops = stops;
        stopsUniform = stops.map(hex => {
          const c = new Color(hex);
          return [c.r, c.g, c.b];
        });
      }
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uColorStops.value = getStopsUniform();
      renderer.render({ scene: mesh });
    };

    // Only run the render loop while the canvas is actually on screen; the
