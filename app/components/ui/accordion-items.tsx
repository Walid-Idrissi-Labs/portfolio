      ...
      <Accordion type="single" collapsible className="">
        {items.map((item) => (
          <AccordionItem value={item.id} key={item.id} className="last:border-b">
            <AccordionTrigger className="cursor-pointer overflow-hidden pl-6 text-left duration-300 ease-in-out hover:no-underline -space-y-5 data-[state=open]:space-y-0 md:pl-14 [&>svg]:hidden [&_h2]:text-[#d9d9d9] [&_h2]:transition-colors [&_h2]:duration-200 [&_p]:text-[#d9d9d9] [&_p]:transition-colors [&_p]:duration-200 data-[state=open]:[&_h2]:text-beige_bright data-[state=open]:[&_p]:text-beige_bright/90">
