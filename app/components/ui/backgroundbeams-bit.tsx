...
// Deterministic per-beam "randomness": keeps the organic spread of durations
// and delays while staying pure during render (and identical on server/client).
  ...
  (
    {
      ...
      intensity = 1,
      duration = 10,
      beamColors = { primary: "#18CCFC", mid: "#6344F5", end: "#AE48FF" },
    }: {
      className?: string;
      isStatic?: boolean;
      /** 0..1 multiplier for how many beams to render (fraction of available paths) */
      intensity?: number;
      /** base duration (seconds) for the animated gradients */
      duration?: number;
      beamColors?: { primary?: string; mid?: string; end?: string };
    },
  ) => {
    const containerRef = useRef<HTMLDivElement>(null);
    // Only run the 50 gradient animations while the section is on screen;
    // offscreen they render as static gradients, which is indistinguishable.
              ...
              animated ? (
                <motion.linearGradient
                  ...
                  animate={{
                    ...
                    x2: ["0%", "95%"],
                    y1: ["0%", "100%"],
                    y2: ["0%", `${93 + pseudoRandom(index, 1) * 8}%`],
                  }}
                  transition={{
                    duration: duration + pseudoRandom(index, 2) * 3,
                    ease: "easeInOut",
                    repeat: Infinity,
                    delay: pseudoRandom(index, 3) * duration,
                  }}
                >
                  <stop stopColor={primary} stopOpacity="0"></stop>
                  <stop stopColor={primary}></stop>
                  <stop offset="32.5%" stopColor={mid}></stop>
                  <stop offset="100%" stopColor={end} stopOpacity="0"></stop>
                </motion.linearGradient>
              ) : (
                <linearGradient
                  id={`linearGradient-${index}`}
